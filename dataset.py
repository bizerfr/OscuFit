...
class PCATrans(object):
    ...
    def __call__(self, data):
        ...
        data['points_patch'] = pts - cp_new
        data['pca_trans'] = trans
        if 'center_normals' in data:
            data['center_normals'] = torch.matmul(data['center_normals'], trans)
        if 'normals_patch' in data:
            data['normals_patch'] = torch.matmul(data['normals_patch'], trans)
        if 'center_directions' in data:
            data['center_directions'] = torch.matmul(
                data['center_directions'].view(2, 3),  # (2, 3)
                trans                                 # (3, 3)
                ).view(6) 
        if 'directions_patch' in data:
            N = data['directions_patch'].shape[0]     # (N, 6)
            data['directions_patch'] = torch.matmul(
                    data['directions_patch'].view(N, 2, 3),  # (N, 2, 3)
                    trans                                    # (3, 3)
                ).view(N, 6)                                 # → back to (N, 6)              
...
class PointCloudDataset(Dataset):
    ...
    def load_data(self, cur_sets):
        for s in tqdm(cur_sets, desc='Loading data'):
            points = load_data(filedir=self.data_dir, filename='%s.xyz' % s, dtype=np.float32)[:, :3]
            ...
            if os.path.exists(os.path.join(self.data_dir, s + '.normals')):
                ...
                nor = normalize(nor, norm='l2', axis=1)
            else:
                nor = np.zeros_like(points)

            if os.path.exists(os.path.join(self.data_dir, s + '.curv')):
                curvatures = load_data(filedir=self.data_dir, filename=s + '.curv', dtype=np.float32)
            else:
                curvatures = np.zeros_like(points)

            if os.path.exists(os.path.join(self.data_dir, s + '.dir')):
                directions = load_data(filedir=self.data_dir, filename=s + '.dir', dtype=np.float32)
                d1 = normalize(directions[:, 0:3], norm='l2', axis=1)
                d2 = normalize(directions[:, 3:6], norm='l2', axis=1)
                directions = np.concatenate([d1, d2], axis=1)
            else:
                directions = np.zeros((points.shape[0], 6), dtype=points.dtype)

            self.pointclouds.append(points)
            self.normals.append(nor)
            self.curvatures.append(curvatures)
            self.directions.append(directions)
            self.shape_names.append(s)
            # KDTree construction may run out of recursions
            sys.setrecursionlimit(int(max(1000, round(points.shape[0]/10))))
            ...
            if self.sparse_patches:
                ...
                self.shape_patch_count.append(points.shape[0])
    ...
    def __getitem__(self, idx):
        ...
        data = {
            'points': self.pointclouds[idx].copy(),
...
class PatchDataset(Dataset):
    ...
    def make_patch(self, points, kdtree=None, normals=None, curvatures=None, directions=None, seed_idx=None, patch_size=1):
        """
        Args:
            points: (N, 3)
            kdtree:
            ...
            patch_size: K
        Returns:
            point_patch, nor_pat: (P, K, 3)
        """
        seed_point = points[seed_idx, :]
        dists, pat_idx = kdtree.query(seed_point, k=patch_size)  # sorted by distance (nearest first)
        patch_radius = max(dists).astype(np.float32)

        points_patch = points[pat_idx, :]        # (K, 3)
        points_patch = points_patch - seed_point    # center
        points_patch = points_patch / patch_radius     # normlize

        normals_patch = normals[pat_idx, :]
        center_normals = normals[seed_idx, :]

        curvatures_patch = None
        center_curvatures = None
        if curvatures is not None:
            curvatures_patch = curvatures[pat_idx, :]
            curvatures_patch = curvatures_patch * patch_radius
            center_curvatures = curvatures[seed_idx, :] * patch_radius

        directions_patch = None
        center_directions = None
        if directions is not None:
            directions_patch = directions[pat_idx, :]
            center_directions = directions[seed_idx, :] 

        return (points_patch, normals_patch, patch_radius, curvatures_patch, 
                directions_patch, center_normals, center_curvatures, center_directions)
    ...
    def __getitem__(self, idx):
        ...
        (points_patch, normals_patch, patch_radius, 
          curvatures_patch, directions_patch, 
          center_normals, center_curvatures, center_directions) = self.make_patch(points=shape_data['points'],
                                                kdtree=shape_data['kdtree'],
                                                normals=shape_data['normals'],
                                                curvatures=shape_data['curvatures'],
                                                directions=shape_data['directions'],
                                                seed_idx=center_point_idx,
                                                patch_size=self.patch_size,
                                                )
        data = {
            'points_patch': torch.from_numpy(points_patch),
            'normals_patch': torch.from_numpy(normals_patch),
            'curvatures_patch': torch.from_numpy(curvatures_patch), # already scaled in making patch
            'directions_patch': torch.from_numpy(directions_patch),
            'patch_radius' : patch_radius,
            'center_normals': torch.from_numpy(center_normals),
            'center_curvatures': torch.from_numpy(center_curvatures),
            'center_directions': torch.from_numpy(center_directions),
